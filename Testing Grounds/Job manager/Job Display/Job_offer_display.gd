...
var job_id_for_displayed_offer:int
...
func update_display() ->void:
	...
	# check if job_offer_display dictionary is empty (no offers currently on display)
	if job_offer_display_order.size() == 0:
		# since no offers are currently displayed 
		pass
	
	## general case ##
	remove_old_offers(current_job_offers)
#	add_new_offers(get_diff(current_job_offers))
	pass
...
func remove_old_offers(current_offers:Dictionary) -> void:
	...
	# at the end make sure the current job_id is still in the list. if not then select the next job offer over
	pass
...
func get_diff(current_offers:Dictionary) ->Array:
	...
	"""
	return []
func add_new_offers(new_offers:Array):
	"""
