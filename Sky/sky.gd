...
func disable_global_light():
	remove_child($sun_moon)

func enable_global_light():
	add_child($sun_moon)

func _input(event):

	if Input.is_action_just_pressed("Cloud_Decrease"):
		clouds_coverage-= 0.05
		
		if clouds_coverage < 0:
			clouds_coverage = 0
		sky_shader.set_shader_parameter("COVERAGE",1.0-(clouds_coverage*0.5+0.25));
		sky_shader.set_shader_parameter("ABSORPTION",clouds_coverage+0.75);
		sky_shader.set_shader_parameter("THICKNESS",clouds_coverage*10.0+10.0);
	if Input.is_action_just_pressed("Cloud_Increase"):
		clouds_coverage+= 0.05
